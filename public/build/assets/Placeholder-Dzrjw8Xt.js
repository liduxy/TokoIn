import{r as i,b as y,j as e,H as O}from"./app-CYqKaw4z.js";import{A as J}from"./AuthenticatedLayout-DMFvbyKh.js";import"./transition-CEqRtjvp.js";function V(){const[j,q]=i.useState([]),[K,T]=i.useState([]),[r,x]=i.useState([]),[m,M]=i.useState(""),[c,N]=i.useState(null),[v,k]=i.useState(!1),[B,g]=i.useState(!1),[h,w]=i.useState(0),[d,p]=i.useState(0),[u,b]=i.useState("cash"),[S,C]=i.useState(""),[_,D]=i.useState(""),[a,R]=i.useState(null),[E,$]=i.useState(!1);i.useEffect(()=>{y.get("/api/kasir/categories").then(t=>T(t.data))},[]);const I=i.useCallback(async()=>{const t=new URLSearchParams;m&&t.set("search",m),c&&t.set("category_id",c.toString());const l=await y.get(`/api/kasir/products?${t}`);q(l.data)},[m,c]);i.useEffect(()=>{I()},[I]);const H=t=>{x(l=>{const n=l.find(s=>s.product_id===t.id);return n?t.track_stock&&n.qty>=t.stock?(alert("Stok tidak mencukupi!"),l):l.map(s=>s.product_id===t.id?{...s,qty:s.qty+1,subtotal:(s.qty+1)*s.price}:s):[...l,{product_id:t.id,name:t.name,price:t.price,qty:1,subtotal:t.price,stock:t.stock,track_stock:t.track_stock}]})},L=(t,l)=>{if(l<1){P(t);return}x(n=>n.map(s=>s.product_id!==t?s:s.track_stock&&l>s.stock?(alert("Stok tidak mencukupi!"),s):{...s,qty:l,subtotal:l*s.price}))},P=t=>{x(l=>l.filter(n=>n.product_id!==t))},f=r.reduce((t,l)=>t+l.subtotal,0),o=f-h,z=async()=>{var t,l;if(r.length===0)return alert("Keranjang kosong!");if(d<o)return alert("Pembayaran kurang!");k(!0);try{const n=await y.post("/api/kasir/transactions",{items:r.map(s=>({product_id:s.product_id,qty:s.qty})),discount:h,paid:d,payment_method:u,customer_name:S||void 0,note:_||void 0});R(n.data),$(!0),g(!1),A()}catch(n){alert(((l=(t=n.response)==null?void 0:t.data)==null?void 0:l.message)||"Gagal memproses transaksi")}finally{k(!1)}},A=()=>{x([]),w(0),p(0),b("cash"),C(""),D("")},F=()=>{var n;const t=window.open("","_blank","width=400,height=600");if(!t||!a)return;const l=`
            <html>
                <head>
